module PastQuestionsHelper
  def base64_image_data(path)
    # path1 = @past_question.image.current_path
    # path1.slice!("#{Rails.root.join("app/assets/images")}/")
    # path1
    ext = File.extname(path)
    if(ext == '.pdf')
      'data:application/pdf;base64,' + Base64.strict_encode64(File.binread(path))
    elsif(ext == '.jpg' || ext == '.jpeg')
      'data:image/jpg;base64,' + Base64.strict_encode64(File.binread(path))
    elsif(ext == '.png')
